...
sim_thresholds = [
    0.9
]  # the similarity between two near duplicates. To test more in this way [0.9, 0.85, 0.7]
...
for (
    sim_threshold
) in sim_thresholds:  # create MinHash for once, when testing multiple similarity values
    ...
    for idx, mh in minhash_dict.items():
        ...
        if len(one_group_ids_int) > group_size:
            exp_id_groups.append(
                one_group_ids_int
            )  # only keep a group with enough sentences
        for i in one_group_ids_int:
            queried_ids.add(i)
    pickle.dump(
        exp_id_groups, open(directory + "groups{}.pickle".format(sim_threshold), "wb")
    )
